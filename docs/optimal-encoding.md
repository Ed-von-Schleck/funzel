...
| | status |
|---|---|
| Certificate-driven placement beats random initialization | **[V]** large effect, both experiments |
...
**[A] The ingredient that works is placement, not relaxation.** Certificate-driven greedy at
$\lambda=0$ — matched-filter placement plus local refinement, i.e. **matching pursuit** — is
what wins. In §9.1 the same greedy recovered four atoms exactly where random restarts scored
100%. Adding $\ell_1$ on top degrades quality on every target tested.
